		...
		<tr class="{cycle values=",alt"}">
			<td class="mid">
				...
				<div class="movextra">
					...
					<a href="{$smarty.const.WWW_TOP}/browse?g={$result.group_name}" title="Browse releases in {$result.group_name|replace:"alt.binaries":"a.b"}">[Grp]</a>
				</div>
			</td>
			<td colspan="7" class="left" id="guid{$result.guid}">
				...
				<div class="movextra">
					<b>{$result.searchname|escape:"htmlall"}</b> <a href="{$smarty.const.WWW_TOP}/movies?imdb={$result.imdbID}" title="View similar nzbs">[Similar]</a>
					{if $isadmin}
						<a href="{$smarty.const.WWW_TOP}/admin/release-edit.php?id={$result.ID}&amp;from={$smarty.server.REQUEST_URI|escape:"url"}" title="Edit Release">[Edit</a> <a class="confirm_action" href="{$smarty.const.WWW_TOP}/admin/release-delete.php?id={$result.ID}&amp;from={$smarty.server.REQUEST_URI|escape:"url"}" title="Delete Release">Del</a> <a class="confirm_action" href="{$smarty.const.WWW_TOP}/admin/release-rebuild.php?id={$result.ID}&amp;from={$smarty.server.REQUEST_URI|escape:"url"}" title="Rebuild Release - Delete and reset for reprocessing if binaries still exist.">Reb]</a>
					{/if}
					<br />
