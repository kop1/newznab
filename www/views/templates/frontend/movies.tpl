...
<table style="width:100%;margin-top:10px;" class="data highlight icons">
	<tr>
		...
		<th width="500">name<br/><a title="Sort Descending" href="{$orderbyname_desc}"><img src="{$smarty.const.WWW_TOP}/views/images/sorting/arrow_down.gif" alt="" /></a><a title="Sort Ascending" href="{$orderbyname_asc}"><img src="{$smarty.const.WWW_TOP}/views/images/sorting/arrow_up.gif" alt="" /></a></th>
	...
	{foreach from=$results item=result}
		<tr class="{cycle values=",alt"}" id="guid{$result.guid}">
			...
			<td>
				{if $result.cover == 1}
					<img src="{$smarty.const.WWW_TOP}/views/images/covers/{$result.imdbID}-cover.jpg" width="100" hspace="10" class="cover" alt="{$result.title|escape:"htmlall"}" align="left" />
				{else}
					<img src="{$smarty.const.WWW_TOP}/views/images/covers/no-cover.jpg" width="100" hspace="10" class="cover" alt="{$result.title|escape:"htmlall"}" align="left" />
				{/if}
				<a class="title" title="View details" href="{$smarty.const.WWW_TOP}/details/{$result.searchname|escape:"htmlall"}/viewnzb/{$result.guid}">{$result.title}</a>
				<br />
				{if $result.tagline != ''}{$result.tagline}<br />{/if}
				<small>{$result.plot}</small>
				<br /><br />		
				<div class="resextra">
					{$result.searchname|escape:"htmlall"}<br />
