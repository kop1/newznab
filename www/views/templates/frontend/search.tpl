...
<form method="get" action="{$smarty.const.WWW_TOP}/search">
	<div style="text-align:center;">
		...
		{if $category[0]}<input type="hidden" name="t" value="{$category[0]}" />{/if}
...
<table style="width:100%;" class="data highlight icons">
	...
	{foreach from=$results item=result}
		<tr class="{cycle values=",alt"}" id="guid{$result.guid}">
			<td><input id="chk{$result.guid|substr:0:7}" type="checkbox" class="nzb_check" name="id[]" value="{$result.guid}" /></td>
			<td>
