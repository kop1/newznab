			...
			<p>
				<ul style="text-align: left;">
					<li>
						Full site feed<br/>
						<a href="{$scheme}{$smarty.server.SERVER_NAME}{$port}/rss?t=0&dl=1&i={$userdata.ID}&r={$userdata.rsstoken}">{$scheme}{$smarty.server.SERVER_NAME}{$port}/rss/?t=0&dl=1&i={$userdata.ID}&r={$userdata.rsstoken}</a>
					</li>
					{foreach from=$categorylist item=category}
						<li>
							{$category.title} feed <br/>
							<a href="{$scheme}{$smarty.server.SERVER_NAME}{$port}/rss?t={$category.ID}&dl=1&i={$userdata.ID}&r={$userdata.rsstoken}">{$scheme}{$smarty.server.SERVER_NAME}{$port}/rss/?t={$category.ID}&dl=1&i={$userdata.ID}&r={$userdata.rsstoken}</a>
						</li>
					{/foreach}
				</ul>
			</p>
